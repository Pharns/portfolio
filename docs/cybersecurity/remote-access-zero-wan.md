...
**TL;DR:** Secure, egress-only remote access using GL.iNet/OpenWrt routers, Tailscale mesh, and self-hosted RustDesk—no WAN exposure or port forwards. Demonstrates IAM discipline, subnet routing/exit modes, and hardening for compliance-friendly remote ops.
